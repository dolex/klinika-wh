...
if ( ! function_exists( 'klinika_setup' ) ) :
	...
	function klinika_setup() {
		...
		add_theme_support( 'custom-background', apply_filters( 'klinika_custom_background_args', array(
...
function klinika_scripts() {
	wp_enqueue_style( 'klinika-style', get_stylesheet_uri() );

	wp_enqueue_script( 'klinika-navigation', get_template_directory_uri() . '/js/navigation.js', array(), '20151215', true );

	wp_enqueue_script( 'klinika-skip-link-focus-fix', get_template_directory_uri() . '/js/skip-link-focus-fix.js', array(), '20151215', true );
...
add_action( 'wp_enqueue_scripts', 'klinika_scripts' );
	...
	function excerpt_word_limit($length) {
		...
		function new_excerpt_more($more) {
			global $post;
			return '<a href="'. get_permalink($post->ID) . '">Читать статью</a>';
		}


function klina_customize_register( $wp_customize ) {
			
			$wp_customize->add_section('section_block', array(
				'title' => __('Настройки сайта','klinika'),
				'priority' => 30,
			));
			...
			$wp_customize->add_section('feature_images', array(
				'title'           => __('Настройка СЛАЙДЕРА', 'klinika'),
		...
		$wp_customize->add_control('feature_product_one_control_text_url', array(
			'label' => __('введите адрес', 'klinika'),
			'section' => 'feature_images',
	...
	add_action( 'customize_register', 'klina_customize_register' );
